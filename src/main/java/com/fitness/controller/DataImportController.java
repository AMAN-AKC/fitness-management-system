...
public class DataImportController {
	 ...
	 * 
	 * Example:
	 * John Doe,john@example.com,9876543210,15-05-1990,123 Main St,Jane
	 * Doe,9876543211,1,,
	 * 
	...
	public ResponseEntity<Map<String, Object>> importMembersFromCsv(
		...
		// Process CSV file
		BulkImportReport report = csvImportService.importMembers(file, dryRun);
	...
	public ResponseEntity<String> downloadImportTemplate() {
		String template = "memName,email,phone,dob,address,emgContact,emgPhone,homeBranchId,referralCode,corporateCode,notes\n"
				+
				"John Doe,john@example.com,9876543210,15-05-1990,123 Main St,Jane Doe,9876543211,1,,,\n" +
				"Jane Smith,jane@example.com,9876543212,20-03-1992,456 Oak Ave,John Smith,9876543213,1,,,High value client\n";
	...
	public ResponseEntity<Map<String, Object>> getImportRules() {
		...
		fieldRules.put("dob", "Required, format: dd-MM-yyyy (e.g., 15-05-1990)");
		...
		fieldRules.put("homeBranchId", "Optional, valid branch ID from system (leave blank for Global Access)");
	...
	@PostMapping("/plans/csv")
	@PreAuthorize("hasAnyRole('ADMIN')")
	@Operation(summary = "Import plans from CSV file with validation report")
	public ResponseEntity<Map<String, Object>> importPlansFromCsv(
			@RequestParam("file") MultipartFile file,
			@RequestParam(value = "dryRun", defaultValue = "false") boolean dryRun) {
		...
		BulkImportReport report = csvImportService.importPlans(file, dryRun);
		return buildImportResponse(report);
	}

	@GetMapping("/plans/template")
	@PreAuthorize("hasAnyRole('ADMIN')")
	@Operation(summary = "Download CSV template for plans import")
	public ResponseEntity<String> downloadPlansImportTemplate() {
		String template = "planName,durationDays,price,accessStart,accessEnd,eligibilityType,prorationRule,taxPercent,branchVisibility\n"
				+ "Annual Pro,365,12000.00,06:00,22:00,GENERAL,PRO_RATA,18.00,ALL\n"
				+ "Student Flex,30,1000.00,10:00,18:00,STUDENT,STRICT,18.00,ALL\n";

		return ResponseEntity.ok()
				.header("Content-Disposition", "attachment; filename=\"plans_import_template.csv\"")
				.header("Content-Type", "text/csv")
				.body(template);
	}
	...
	@PreAuthorize("hasAnyRole('ADMIN')")
	@Operation(summary = "Import classes from CSV file with validation report")
	public ResponseEntity<Map<String, Object>> importClassesFromCsv(
			@RequestParam("file") MultipartFile file,
			@RequestParam(value = "dryRun", defaultValue = "false") boolean dryRun) {
		...
		BulkImportReport report = csvImportService.importClasses(file, dryRun);
		return buildImportResponse(report);
	}

	@GetMapping("/classes/template")
	@PreAuthorize("hasAnyRole('ADMIN')")
	@Operation(summary = "Download CSV template for classes import")
	public ResponseEntity<String> downloadClassesImportTemplate() {
		String template = "className,trainerId,roomId,branchId,startDate,endDate,weekdays,classTime,durationMins,capacity,prerequisites,planEligibility\n"
				+ "Yoga Basics,1,2,1,2026-06-01,2026-12-31,MONDAY,08:00,60,20,,GENERAL\n";
	...
	@PostMapping("/schedule/csv")
	@PreAuthorize("hasAnyRole('ADMIN')")
	@Operation(summary = "Import schedule from CSV file with validation report")
	public ResponseEntity<Map<String, Object>> importScheduleFromCsv(
			@RequestParam("file") MultipartFile file,
			@RequestParam(value = "dryRun", defaultValue = "false") boolean dryRun) {
		...
		BulkImportReport report = csvImportService.importSchedule(file, dryRun);
		return buildImportResponse(report);
	}

	@GetMapping("/schedule/template")
	@PreAuthorize("hasAnyRole('ADMIN')")
	@Operation(summary = "Download CSV template for schedule import")
	public ResponseEntity<String> downloadScheduleImportTemplate() {
		String template = "classId,trainerId,roomId,date,time,status\n"
				+ "1,1,2,2026-06-01,08:00,ACTIVE\n";

		return ResponseEntity.ok()
				.header("Content-Disposition", "attachment; filename=\"schedule_import_template.csv\"")
				.header("Content-Type", "text/csv")
				.body(template);
	}

	private ResponseEntity<Map<String, Object>> buildImportResponse(BulkImportReport report) {
		Map<String, Object> response = new HashMap<>();
		response.put("success", true);
		response.put("message", report.getSummary());
		response.put("fileName", report.getFileName());
